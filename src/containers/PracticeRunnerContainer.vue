...
<template>
  <div class="practice-runner-container" :class="[`tone-${schema.tone || 'high_focus'}`]">
...
<style scoped>
.practice-runner-container {
  position: relative;
  min-height: 100vh;
  width: 100%;
  /* Spec: static deep blue #0B1220 */
  background: #0B1220;
  display: flex;
...
.active-stage {
  ...
  margin: 0 auto;
  text-align: center;
  animation: practice-enter 1s ease-out;
}
...
/* Typography Overrides for Focus Mode */
:deep(.micro-label) {
  font-family: var(--font-sans);
  font-size: 12px;
  letter-spacing: 2px;
